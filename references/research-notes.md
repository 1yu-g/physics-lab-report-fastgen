...
- [Microsoft MarkItDown](https://github.com/microsoft/markitdown) (MIT): its common document-to-Markdown interface informed the lightweight local material bundle. FastGen uses smaller deterministic parsers for the course formats it needs.
- [Docling](https://github.com/docling-project/docling) (MIT): its unified document representation and provenance model informed the per-page, per-slide, and per-sheet JSON units. Docling remains an optional future backend for complex scanned layouts.
- [PaddleOCR PP-StructureV3](https://github.com/PaddlePaddle/PaddleOCR) (Apache-2.0): its table, formula, and reading-order pipeline is documented as a heavier fallback for complex records; the default remains the faster img2table + RapidOCR route.
